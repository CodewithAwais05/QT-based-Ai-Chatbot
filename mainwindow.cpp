...
#include <QFileDialog>      // NEW: for open-file dialogs
#include <QPixmap>          // NEW: for rendering image thumbnails
    ...
    typingDots(0), isRecording(false), isDarkMode(true)
{
    ...
    // ------------------------API Key here-------------------------------------
    QString apiKey = "Paste-Your-API-KEY-HERE";
    chatbot = new AIChatbot(apiKey, this);

    connect(chatbot, &AIChatbot::responseReady,  this, &MainWindow::onBotResponse);
    connect(chatbot, &AIChatbot::errorOccurred,  this, &MainWindow::onBotError);
    connect(chatbot, &AIChatbot::typingStarted,  this, &MainWindow::onTypingStarted);
...
void MainWindow::setupLoginPage() {
    ...
    // Updated feature list includes new capabilities
...
void MainWindow::setupChatPage() {
    ...
    connect(deleteBtn, &QPushButton::clicked, this, &MainWindow::onDeleteChat);
    ...
    QLabel* wSub = new QLabel(
        "Type, use 🎤 voice, upload 🖼️ images or 📎 files — I am here to assist!");
    ...
    // ── Input area ────────────────────────────────────────
    // Outer container: stacks attachment strip + input row
    QWidget* inputArea = new QWidget();
    inputArea->setObjectName("inputArea");
    QVBoxLayout* inputAreaLayout = new QVBoxLayout(inputArea);
    inputAreaLayout->setContentsMargins(20, 8, 20, 12);
    inputAreaLayout->setSpacing(6);

    // ── NEW: Attachment indicator strip ───────────────────
    // Shows the name of the pending attachment (image/file).
    // Hidden until the user picks a file.
    attachmentLabel = new QLabel("");
    attachmentLabel->setObjectName("attachmentLabel");
    attachmentLabel->setWordWrap(false);
    attachmentLabel->hide();
    attachmentLabel->setStyleSheet(
        "background:#1e2640; border-radius:8px; padding:4px 10px;"
        "font-size:12px; color:#a78bfa; border:1px solid #2a3060;");

    // Button row
    QWidget* buttonRow = new QWidget();
    QHBoxLayout* inputLayout = new QHBoxLayout(buttonRow);
    ...
    emojiBtn->setFixedSize(46, 46);
    emojiBtn->setCursor(Qt::PointingHandCursor);
    emojiBtn->setToolTip("Emoji picker");
    connect(emojiBtn, &QPushButton::clicked, this, &MainWindow::onShowEmojiPicker);

    // Voice button
    micBtn = new QPushButton("🎤");
    micBtn->setObjectName("micBtn");
    micBtn->setFixedSize(46, 46);
    micBtn->setCursor(Qt::PointingHandCursor);
    micBtn->setToolTip("Click to record voice");
    connect(micBtn, &QPushButton::clicked, this, &MainWindow::onMicButtonClicked);

    // NEW: Upload Image button
    uploadImageBtn = new QPushButton("🖼️");
    uploadImageBtn->setObjectName("micBtn");
    uploadImageBtn->setFixedSize(46, 46);
    uploadImageBtn->setCursor(Qt::PointingHandCursor);
    uploadImageBtn->setToolTip("Upload an image (PNG, JPG, WEBP...)");
    connect(uploadImageBtn, &QPushButton::clicked, this, &MainWindow::onUploadImage);

    // NEW: Upload File button
    uploadFileBtn = new QPushButton("📎");
    uploadFileBtn->setObjectName("micBtn");
    uploadFileBtn->setFixedSize(46, 46);
    uploadFileBtn->setCursor(Qt::PointingHandCursor);
    uploadFileBtn->setToolTip("Upload a file (PDF, TXT, DOCX...)");
    connect(uploadFileBtn, &QPushButton::clicked, this, &MainWindow::onUploadFile);

    // Text input
    messageInput = new QTextEdit();
    messageInput->setObjectName("messageInput");
    messageInput->setPlaceholderText("Type a message, or attach 🖼️ / 📎 ...");
    ...
    inputLayout->addWidget(uploadImageBtn, 0, Qt::AlignBottom);   // NEW
    inputLayout->addWidget(uploadFileBtn,  0, Qt::AlignBottom);   // NEW
    inputLayout->addWidget(messageInput);
    inputLayout->addWidget(sendBtn,        0, Qt::AlignBottom);

    inputAreaLayout->addWidget(attachmentLabel);   // NEW strip
    inputAreaLayout->addWidget(buttonRow);
...
// ═══════════════════════════════════════════════════════════
// STYLES (if we want to change color or something in gui then do it in this section)
// ═══════════════════════════════════════════════════════════
void MainWindow::applyGlobalStyles()
{
    setStyleSheet(
        ...
        "#logoutBtn:hover,#deleteBtn:hover {"
        "background:#450A0A;"
        "}"
                                  ...
                                  bool isUser)
{
    ...
    QLabel* time = new QLabel(QDateTime::currentDateTime().toString("hh:mm ap"));
...
// ─── NEW: addImageBubble ──────────────────────────────────
// Shows the image inline in the chat as a thumbnail.
void MainWindow::addImageBubble(const QString& imagePath, bool isUser)
{
    QWidget* row = new QWidget();
    row->setStyleSheet("background: transparent;");
    QHBoxLayout* rowLayout = new QHBoxLayout(row);
    ...
    avatar->setFixedSize(36, 36);
    avatar->setAlignment(Qt::AlignCenter);
    avatar->setStyleSheet("background:#1e2640; border-radius:18px; font-size:16px;");

    // Render image thumbnail (max 280 × 200 px)
    ...
    QLabel* senderLabel = new QLabel("You");
    senderLabel->setStyleSheet("color:#7c83fd; font-size:11px; font-weight:600;");
...
void MainWindow::clearPendingAttachment() {
...
// ═══════════════════════════════════════════════════════════
// SEND MESSAGE (handles plain text, image, and file paths)
// ═══════════════════════════════════════════════════════════
void MainWindow::onSendMessage() {
    QString text = messageInput->toPlainText().trimmed();

    // At least one of text or attachment must be present
    if (text.isEmpty() && pendingImagePath.isEmpty() && pendingFilePath.isEmpty())
        return;
    ...
    if (!pendingImagePath.isEmpty()) {
        // ── Image path ─────────────────────────────────────
        addImageBubble(pendingImagePath, true);
        if (!text.isEmpty())
            addMessageBubble("You", text, true);

        QString imagePath = pendingImagePath;
        clearPendingAttachment();
        chatbot->sendMessageWithImage(text, imagePath);

    } else if (!pendingFilePath.isEmpty()) {
        // ── File path ──────────────────────────────────────
        QString label = FileUploader::getFileLabel(pendingFilePath);
        addMessageBubble("You",
                         (text.isEmpty() ? "" : text + "\n") + label,
                         true);

        QString filePath = pendingFilePath;
        clearPendingAttachment();
        chatbot->sendMessageWithFile(text, filePath);
...
// The chosen path is stored in pendingImagePath and shown
// in the attachment strip above the input box.
void MainWindow::onUploadImage() {
    ...
    QString label = FileUploader::getFileLabel(path);
    attachmentLabel->setText("📎  " + label + "   ✕ click image button again to cancel");
    attachmentLabel->show();

    // Visual feedback on the upload button
    botStatusLabel->setText("🖼️ Image ready — type a question or just hit Send");
...
// ─── onUploadFile ─────────────────────────────────────────
// Opens a file dialog for document selection.
// Extracted text is sent as context with the next message.
void MainWindow::onUploadFile() {
    // If an image is already queued, clicking file button cancels it
    if (!pendingImagePath.isEmpty()) {
        clearPendingAttachment();
        botStatusLabel->setText("Attachment cleared.");
        QTimer::singleShot(2000, this, [this]() { botStatusLabel->setText("● Online"); });
        return;
    }
    // If a file is already queued, cancel it
    ...
    QString path = QFileDialog::getOpenFileName(
        ...
        "Select a File",
        QString(),
        "Documents (*.txt *.md *.csv *.log *.pdf *.docx)"
        );
    if (path.isEmpty()) return;
    ...
    QString label = FileUploader::getFileLabel(path);
    attachmentLabel->setText("📎  " + label + "   ✕ click 📎 again to cancel");
    attachmentLabel->show();

    botStatusLabel->setText("📄 File ready — type a question or just hit Send");
    QTimer::singleShot(3000, this, [this]() {
        botStatusLabel->setText("● Online");
    });
}
...
void MainWindow::loadHistoryList() {
    ...
    for (const QString& sid : sessions) {
        ...
        card->setStyleSheet(
            ...
            "QWidget:hover { border:1px solid #7c83fd; background:#1a2040; }"
            );
        ...
        sessionLabel->setStyleSheet("color:#7c83fd; font-size:11px; font-weight:600;");
        QLabel* countLabel = new QLabel(QString::number(msgs.size()) + " messages");
...
void MainWindow::onShowEmojiPicker() {
    ...
    if (emojiPickerVisible) {
        QPoint pos = emojiBtn->mapTo(
            chatPage, QPoint(0, -emojiPicker->sizeHint().height() - 10));
